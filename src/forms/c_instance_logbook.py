...
class CInstanceLogbookAdd(Form):
    ...
    async def is_valid(self):
        ...
        if not self.version_id or not self.version_id.isnumeric() or int(self.version_id) == 0:
            self.errors.setdefault("version_id", self.REQUIRED_ERROR)

        if not self.equipment_id or not self.equipment_id.isnumeric() or int(self.equipment_id) == 0:
            self.errors.setdefault("equipment_id", self.REQUIRED_ERROR)

        if not self.responsible_user_id or not self.responsible_user_id.isnumeric() or int(self.responsible_user_id) == 0:
        ...
        if self.responsible_user_id in [self.head_commision_member_id, self.commision_member_id, self.performer_id]:
            self.errors.setdefault("responsible_user_id", "Пользователь СКЗИ не может быть в составе комиссии")
        ...
        if self.key_document_is_unexpired == "on":
            ...
            if self.key_document_unexpired_serial:
                is_exists = await KeyDocumentServise.get_one_or_none(serial=self.key_document_unexpired_serial)
                if is_exists:
            ...
            if not self.key_document_unexpired_serial:
                self.errors.setdefault("key_document_unexpired_serial", self.REQUIRED_ERROR)

            if not self.key_document_unexpired_carrier_id or not self.key_document_unexpired_carrier_id.isnumeric() or int(self.key_document_unexpired_carrier_id) == 0:
                self.errors.setdefault("key_document_unexpired_carrier_id", self.REQUIRED_ERROR)

            if not self.key_document_unexpired_received_from:
                self.errors.setdefault("key_document_unexpired_received_from", self.REQUIRED_ERROR)

            if not self.key_document_unexpired_received_at:
                self.errors.setdefault("key_document_unexpired_received_at", self.REQUIRED_ERROR)

        if self.key_document_is_expired == "on" and self.key_document_expired_is_new == "True":

            if self.key_document_expired_serial:
                is_exists = await KeyDocumentServise.get_one_or_none(serial=self.key_document_expired_serial)
                if is_exists:
                    self.errors.setdefault(
            ...
            if not self.key_document_expired_serial:
                self.errors.setdefault("key_document_expired_serial", self.REQUIRED_ERROR)

            if not self.key_document_expired_carrier_id or not self.key_document_expired_carrier_id.isnumeric() or int(self.key_document_expired_carrier_id) == 0:
                self.errors.setdefault("key_document_expired_carrier_id", self.REQUIRED_ERROR)

            if not self.key_document_expired_received_from:
                self.errors.setdefault("key_document_expired_received_from", self.REQUIRED_ERROR)

            if not self.key_document_expired_received_at:
                self.errors.setdefault("key_document_expired_received_at", self.REQUIRED_ERROR)

        if self.key_document_is_expired == "on" and self.key_document_expired_is_new == "False":

            if not self.key_document_id or not self.key_document_id.isnumeric() or int(self.key_document_id) == 0:
        ...
        if not self.key_document_is_expired and not self.key_document_is_unexpired:
            self.errors.setdefault("key_document", "Установка СКЗИ без набора ключей не имеет смысла, укажите хотя бы один ключевой набор!")
