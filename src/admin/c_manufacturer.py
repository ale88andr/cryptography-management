...
@router.get("/add")
async def add_cmanufacturer_admin(request: Request):
    return templates.TemplateResponse(
        form_teplate,
        {
            "request": request,
...
@router.post("/add")
async def create_cmanufacturer_admin(request: Request):
...
async def edit_cmanufacturer_admin(cm_id: int, request: Request):
...
async def update_carrier_admin(cm_id: int, request: Request):
    form = CManufacturerForm(request)
    await form.load_data()
    if await form.is_valid():
        try:
...
async def delete_cmanufacturer_admin(cm_id: int, request: Request):
